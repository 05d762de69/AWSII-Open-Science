...
`%+%` <- function(a, b) paste0(a, b)   # simple string‑concat helper (define early)
nbsp   <- "\u00A0"                      # real non‑breaking space for PDF & HTML
indent <- function(level = 1) strrep(nbsp, 3 * level)

# ---- data -------------------------------------------------------------------
df <- read.csv("/Users/I743312/Library/CloudStorage/OneDrive-ErasmusUniversityRotterdam/01_Study/02_Second Year/Academic Writing II/Report/Data/679693_Sample MRI Dementia in Older Adults.csv")

# ---- recode & summarise -----------------------------------------------------
df <- df %>%
  mutate(
    Gender     = factor(Gender, labels = c("Male", "Female")),
    CDR_binary = ifelse(CDR == 0, "CDR 0", "CDR 0.5 or 1")
  )
...
#--- helper functions
apa_p <- function(p) {
  # round to 3digits, then format in APA style (no leading zero, <.001 logic)
  p %>% 
    p_round(digits = 3) %>% 
    p_format(accuracy = .001, leading.zero = FALSE, trailing.zero = FALSE, add.p = FALSE, space = TRUE)
}

```

```{r preliminary-analyses, echo=FALSE, warning=FALSE}
# Chi-Square Test
contingency_table <- table(df$Gender,df$CDR_binary)
pct_by_group <- round(prop.table(contingency_table, 2) * 100, 1)
chi_square_test <- chisq.test(contingency_table)

# t-Tests
## Levene's tests
library(car)
levene_age <- leveneTest(Age ~ CDR_binary, df)
levene_educ <- leveneTest(EDUC ~ CDR_binary, df)
levene_mmse <- leveneTest(MMSE ~ CDR_binary, df)

## t-Tests
t_age <- t.test(Age ~ CDR_binary, data = df, var.equal = TRUE)
d_age <- cohen.d(df$Age, df$CDR_binary)
t_educ <- t.test(EDUC ~ CDR_binary, data = df, var.equal = TRUE)
d_educ <- cohen.d(df$EDUC, df$CDR_binary)
t_mmse <- t.test(MMSE ~ CDR_binary, data = df, var.equal = FALSE)
...
# choose alignment per output target -------------------------------
if (knitr::is_latex_output()) {
  stub_align <- "p{8.5cm}"   # wraps nicely in PDF
} else {
  stub_align <- "l"          # plain left align for HTML / Word
}
align_vec <- c(
  "p{2in}",                                 # stub column
...
# Correlations 
cor_all <- cor(df$nWBV, df$MMSE, method = "pearson")
...
library(cocor) 
comparison_cor <- cocor.indep.groups( 
  r1 = r_dementia, 
  r2 = r_nondementia, 
...
The first set of questions aimed to clarify the relationship between nWBV and MMSE scores. It was hypothesized that nWBV and MMSE scores would positively correlate, and that this relationship would be stronger in the dementia group, compared to the non-dementia group. The correlation between nWBV and MMSE scores was tested using *Pearson's r*, and a moderate positive correlation was found ($r=$ `r round(cor_all,2)`). Furthermore, as shown in @fig-correlation, the correlation between nWBV and MMSE scores in the dementia group ($r=$ `r round(r_dementia, 2)`) is significantly larger than the correlation in the non-dementia group ($r=$ `r  round(r_nondementia, 2)`), $z=$ `r round(comparison_cor@fisher1925$statistic,2)`, $p=$ `r apa_p(comparison_cor@fisher1925$p.value)`.
...
library(ggplot2)
library(papaja)          # gives theme_apa()
...
library(glue)
library(papaja)       # apa_table()
library(dplyr)

# ── 0  Fit model and add residuals ──────────────────────────────────────────
model_dementia_age <- aov(nWBV ~ Dementia_groups * Age_groups, data = df)
model_dementia_gender <- aov(nWBV ~ Dementia_groups * Gender, data = df)

df$resid_dementia_age <- residuals(model_dementia_age)
df$resid_dementia_gender <- residuals(model_dementia_gender)

# ── 1  Assumption checks ----------------------
## Levene’s test on the four cells
df$Cell <- interaction(df$Dementia_groups, df$Age_groups, drop = TRUE)
levene_dementia_age <- leveneTest(nWBV ~ Cell, data = df)

df$Cell <- interaction(df$Dementia_groups, df$Gender, drop = TRUE)
levene_dementia_gender <- leveneTest(nWBV ~ Cell, data = df)

## Shapiro–Wilk per cell
shapiro_wilk_test_age <- df %>%
  group_by(Dementia_groups, Age_groups) %>%
  summarise(
    W = shapiro.test(resid_dementia_age)$statistic,
    p = shapiro.test(resid_dementia_age)$p.value,
    .groups = "drop"
  )

shapiro_wilk_test_gender <- df %>%
  group_by(Dementia_groups, Gender) %>%
  summarise(
    W = shapiro.test(resid_dementia_gender)$statistic,
    p = shapiro.test(resid_dementia_gender)$p.value,
    .groups = "drop"
  )

#--- 2 Getting eta
# Get the full result table
res_dementia_age <- apa_print(model_dementia_age)$table
res_dementia_gender <- apa_print(model_dementia_gender)$table

# Extract the "estimate" column for a specific effect
raw_DA_string_dementia <- res_dementia_age$estimate[res_dementia_age$term == "Dementia groups"]
raw_DA_string_age <- res_dementia_age$estimate[res_dementia_age$term == "Age groups"]
raw_DA_string_interaction <- res_dementia_age$estimate[res_dementia_age$term == "Dementia groups $\\times$ Age groups"]

raw_DG_string_dementia <- res_dementia_gender$estimate[res_dementia_gender$term == "Dementia groups"]
raw_DG_string_age <- res_dementia_gender$estimate[res_dementia_gender$term == "Gender"]
raw_DG_string_interaction <- res_dementia_gender$estimate[res_dementia_gender$term == "Dementia groups $\\times$ Gender"]


# Extract the numeric value from the string
eta2_DA_numeric_dementia <- as.numeric(sub(" .*", "", raw_DA_string_dementia))
eta2_DA_numeric_age <- as.numeric(sub(" .*", "", raw_DA_string_age))
eta2_DA_numeric_interaction <- as.numeric(sub(" .*", "", raw_DA_string_interaction))

eta2_DG_numeric_dementia <- as.numeric(sub(" .*", "", raw_DG_string_dementia))
eta2_DG_numeric_gender <- as.numeric(sub(" .*", "", raw_DG_string_age))
eta2_DG_numeric_interaction <- as.numeric(sub(" .*", "", raw_DG_string_interaction))

# 3- Get mean values
cell_stats <- df %>%
  mutate(
    Age_groups = factor(Age_groups, levels = c(0, 1),
                        labels = c("60–70", "70+")),
  ...
  group_by(Dementia_groups, Age_groups) %>%
  summarise(
    Mean_prop = mean(nWBV, na.rm = TRUE),
    SD_prop   = sd(nWBV,   na.rm = TRUE),
    .groups   = "drop"
  )

# ---- marginal means & SDs (proportions) -------------------------
marg_dementia <- cell_stats %>%
  group_by(Dementia_groups) %>%
  summarise(
    Mean_prop = mean(Mean_prop),
    SD_prop   = sqrt(mean(SD_prop^2)),   # pooled within-group SD
    .groups   = "drop"
  )
...
library(dplyr)
library(ggplot2)
library(patchwork)   # arrange side-by-side
library(papaja)
...
df_plot <- df |>
  mutate(
    ...
    Gender          = factor(Gender,
                             levels = c("Male", "Female"))   # adjust if coded 0/1
  )

# helper to get raw means + 95 % CIs -----------------------------------------
summary_ci <- function(data, x, grp) {
  data |>
    group_by({{grp}}, {{x}}) |>
    summarise(
      mean   = mean(nWBV, na.rm = TRUE),
      n      = n(),
...
age_ci    <- summary_ci(df_plot, Age_groups, Dementia_groups)
gender_ci <- summary_ci(df_plot, Gender,     Dementia_groups)

# plotting function -----------------------------------------------------------
plot_interaction <- function(df, xvar, xlabel, title = NULL) {
  ggplot(df,
         aes(x = {{ xvar }},
             y = mean,
             fill = Dementia_groups)) +
...
p_age    <- plot_interaction(age_ci,    Age_groups, "Age group (years)",  "(A)  Age × Dementia Status")
p_gender <- plot_interaction(gender_ci, Gender,     "Gender",             "(B)  Gender × Dementia Status")
...
A statistically significant main effect of dementia status on nWBV was found (`r apa_print(model_dementia_age)$statistic$Dementia_groups`, $\eta p^2=$ `r apa_p(eta2_DA_numeric_dementia)`). On average, the dementia group ($M=$ `r round(marg_dementia$Mean_prop[marg_dementia$Dementia_groups == "Dementia"], 3)`, $SD=$ `r round(marg_dementia$SD_prop[marg_dementia$Dementia_groups == "Dementia"], 3)`) showed a smaller nWBV than the non-dementia group ($M=$ `r round(marg_dementia$Mean_prop[marg_dementia$Dementia_groups == "No dementia"], 3)`, $SD=$ `r round(marg_dementia$SD_prop[marg_dementia$Dementia_groups == "No dementia"], 3)`). Additionally, a statistically significant main effect of age group (`r apa_print(model_dementia_age)$statistic$Age_groups`, $\eta p^2=$ `r apa_p(eta2_DA_numeric_age)`) was found. The interaction effect between dementia status and age group was also statistically significant (`r apa_print(model_dementia_age)$statistic$Dementia_groups_Age_groups`, $\eta p^2=$ `r apa_p(eta2_DA_numeric_interaction)`). Interestingly, as illustrated in panel A of @fig-nwbv-dual, the dementia-related reduction in nWBV was larger in the younger-old group ($\Delta=$ `r round(cell_stats$Mean_prop[1] - cell_stats$Mean_prop[3], 3)`) than in the older group ($\Delta=$ `r round(cell_stats$Mean_prop[2] - cell_stats$Mean_prop[4], 3)`).

A similar analysis was conducted with dementia status and gender as factors to examine whether the effect of dementia status on nWBV differed by gender. This ANOVA revealed a statistically significant main effect of dementia status ( `r apa_print(model_dementia_gender)$statistic$Dementia_groups`, $\eta p^2=$ `r apa_p(eta2_DG_numeric_dementia)`), and a statistically significant main effect of gender (`r apa_print(model_dementia_gender)$statistic$Gender`, $\eta p^2=$ `r apa_p(eta2_DG_numeric_gender)`). However, the interaction effect between dementia status and gender was not statistically significant (`r apa_print(model_dementia_gender)$statistic$Dementia_groups_Gender`, $\eta p^2=$ `r apa_p(eta2_DG_numeric_interaction)`, see panel B of @fig-nwbv-dual).
